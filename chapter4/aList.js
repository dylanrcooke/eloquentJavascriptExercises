 ...
 * structure is the list (not to be confused with array). A list is a nested set of objects, with the first 
 ...
 * they are both independent lists, but they share the structure that makes up their last three elements. The 
...
function arrayToList(input) {
    return input.reverse().reduce((rest, value) => { 
        return { value, rest };
    }, null);
}
...
function listToArray(input) {
    const output = [];
    while(input !== null && input.value !== null) {
...
function nth(input, position) {
    if (position === 0) return input.value;
    else if (input.rest === null) return undefined;
    else return nth(input.rest, position - 1);
}
